<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Add Grade</h2>
                ...
                <form method="POST" action="{{ route('grades.store') }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="credits" id="credits" value="{{ old('credits') }}" step="0.1" min="0" required class="input">
                            @error('credits') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <x-input-label for="score">Score</x-input-label>
                            <input type="number" name="score" id="score" value="{{ old('score') }}" step="0.01" min="0" class="input">
                        ...
                        <div>
                            ...
                            <select name="letter_grade" id="letter_grade" class="input">
                                ...
                                <option value="A+" @selected(old('letter_grade') == 'A+')>A+</option>
                                <option value="A" @selected(old('letter_grade') == 'A')>A</option>
                                <option value="A-" @selected(old('letter_grade') == 'A-')>A-</option>
                                <option value="B+" @selected(old('letter_grade') == 'B+')>B+</option>
                                <option value="B" @selected(old('letter_grade') == 'B')>B</option>
                                <option value="B-" @selected(old('letter_grade') == 'B-')>B-</option>
                                <option value="C+" @selected(old('letter_grade') == 'C+')>C+</option>
                                <option value="C" @selected(old('letter_grade') == 'C')>C</option>
                                <option value="C-" @selected(old('letter_grade') == 'C-')>C-</option>
                                <option value="D+" @selected(old('letter_grade') == 'D+')>D+</option>
                                <option value="D" @selected(old('letter_grade') == 'D')>D</option>
                                <option value="F" @selected(old('letter_grade') == 'F')>F</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="grade_points" id="grade_points" value="{{ old('grade_points') }}" step="0.01" min="0" max="4" class="input">
                        ...
                        <div>
                            ...
                            <select name="semester" id="semester" required class="input">
                                ...
                                <option value="Spring" @selected(old('semester') == 'Spring')>Spring</option>
                                <option value="Summer" @selected(old('semester') == 'Summer')>Summer</option>
                                <option value="Fall" @selected(old('semester') == 'Fall')>Fall</option>
                            </select>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="is_elective" id="is_elective" value="1" @checked(old('is_elective')) class="rounded border-slate-300 dark:border-slate-600 text-cyan-600 focus:ring-cyan-500">
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        ...
                        <button type="submit" class="btn-primary text-sm">Save Grade</button>
                    </div>
                </form>
            </div>
        </div>
