<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">{{ $deck->title }}</h2>
                @if ($deck->topic)
                    <p class="text-sm text-slate-500">{{ $deck->topic->title }}</p>
                @endif
            </div>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            @if ($deck->description)
                <div class="card p-4 text-sm text-slate-500 dark:text-slate-400">{{ $deck->description }}</div>
            @endif

            <div class="card p-6">
                <h3 class="font-semibold text-sm text-slate-900 dark:text-slate-100 mb-4">Add Card</h3>
                <form method="POST" action="{{ route('flashcards.cards.store', $deck) }}" class="space-y-3">
                    @csrf
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-medium text-slate-500 mb-1">Front (Question)</label>
                            <textarea name="front" rows="2" required class="input text-sm"></textarea>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-500 mb-1">Back (Answer)</label>
                            <textarea name="back" rows="2" required class="input text-sm"></textarea>
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-500 mb-1">Hint (optional)</label>
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="font-semibold text-slate-900 dark:text-slate-100 text-sm">{{ $deck->flashcards->count() }} Cards</h3>
                </div>
                <div class="divide-y divide-slate-200/70 dark:divide-slate-700/60">
                    @forelse ($deck->flashcards as $card)
                        <div class="px-6 py-4" x-data="{ show: false }">
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    <p class="font-medium text-sm text-slate-900 dark:text-slate-100">{{ $card->front }}</p>
                                    <div x-show="show" x-transition class="mt-2 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-2xl">
                                        <p class="text-sm text-slate-700 dark:text-slate-300">{{ $card->back }}</p>
                                        @if ($card->hint)
                                            <p class="text-xs text-slate-500 mt-1">Hint: {{ $card->hint }}</p>
                                        @endif
                                    </div>
                                    <p class="text-xs text-slate-500 mt-1">{{ $card->total_reviews }} reviews</p>
                                </div>
                    ...
                    @empty
                        <div class="px-6 py-8 text-center text-sm text-slate-500">No cards yet.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="card p-4">
                <details class="group">
                    <summary class="text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer">Edit Deck</summary>
                    <div class="mt-3 space-y-3">
                        <form method="POST" action="{{ route('flashcards.update', $deck) }}" class="space-y-3">
                            @csrf @method('PUT')
